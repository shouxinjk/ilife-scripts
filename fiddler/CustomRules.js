...
*/
    ...
    static function extractData(oSession: Session){
        ...
        if(oSession.responseCode == 200 && oSession.host.Contains(".lhs11.com")){//旅划算
            extractLvhuasuan(oSession);
        }
    }
        
    //联联周边游
    static function extractLianlianly(oSession: Session){
        var rootDir = "C:\\Users\\alexchew\\Desktop\\lianlian";
        var url = oSession.url;
        if(oSession.url.Contains("/v1/wx/list") || 
        oSession.url.Contains("/v1/wx/around/list") || 
        oSession.url.Contains("/v1/wx/region/list")){//首页列表 
            var path = rootDir+"\\list\\" + new Date().getTime()+"_.json";
            if(!System.IO.File.Exists(path)){
                oSession.utilDecodeResponse();//需要先解码，否则JSON格式不正确
                oSession.SaveResponseBody(path);
                //TODO:可以调用pan执行transform转换完成数据提交
            }
        }
        /**
        //是城市分站列表，不需要处理
        if(oSession.url.Contains("/wx/city/list")){//城市首页列表
            var path = rootDir+"\\list-by-city\\" + new Date().getTime()+"_.json";
            if(!System.IO.File.Exists(path)){
        ...
        if(oSession.url.Contains("/v1/wx/product2")){//产品详情
            var path = rootDir+"\\item\\" + new Date().getTime()+"_.json";
            if(!System.IO.File.Exists(path)){
                oSession.utilDecodeResponse();
                oSession.SaveResponseBody(path);
                //TODO:可以调用pan执行transform转换完成数据提交
        ...
        if(oSession.url.Contains("/v1/wx/product/qrcode")){//海报二维码
            var path = rootDir+"\\qrcode\\" + new Date().getTime()+"_.json";
            if(!System.IO.File.Exists(path)){
                oSession.utilDecodeResponse();
                oSession.SaveResponseBody(path);
                //TODO:可以调用pan执行transform转换完成数据提交
    ...
    //旅划算
    static function extractLvhuasuan(oSession: Session){
        var rootDir = "C:\\Users\\alexchew\\Desktop\\lvhuasuan";
        var url = oSession.url;
        if(oSession.url.Contains("/fxapi/services/v2/shelf@homePageShelfInfo.japi")){//首页列表
            var path = rootDir+"\\list\\" + new Date().getTime()+"_.json";
            if(!System.IO.File.Exists(path)){
                oSession.utilDecodeResponse();//需要先解码，否则JSON格式不正确
                oSession.SaveResponseBody(path);
        ...
        if(oSession.url.Contains("/fxapi/services/system/ctmall/homepage@storeShelfInfo.japi")){//频道列表：如今日上新、美食、酒店
            var path = rootDir+"\\list-by-subhome\\" + new Date().getTime()+"_.json";
            if(!System.IO.File.Exists(path)){
                oSession.utilDecodeResponse();
                oSession.SaveResponseBody(path);
                //TODO:可以调用pan执行transform转换完成数据提交
        ...
        if(oSession.url.Contains("/fxapi/services/v2/shelf@getTagShelfProductRebate.japi")){//标签列表：如即将结束
            var path = rootDir+"\\list-by-tag\\" + new Date().getTime()+"_.json";
            if(!System.IO.File.Exists(path)){
                oSession.utilDecodeResponse();//需要先解码，否则JSON格式不正确
                oSession.SaveResponseBody(path);
                //TODO:可以调用pan执行transform转换完成数据提交
        ...
        if(oSession.url.Contains("/fxapi/services/v2/product@cityProductList.japi")){//城市分页列表
            var path = rootDir+"\\list-by-tag\\" + new Date().getTime()+"_.json";//采用和tag完全相同的数据结构
            if(!System.IO.File.Exists(path)){
                oSession.utilDecodeResponse();
                oSession.SaveResponseBody(path);
                //TODO:可以调用pan执行transform转换完成数据提交
        ...
        if(oSession.url.Contains("/fxc/services/ms/fxc/product@getProductDetail.japi")){//ok产品详情
            var path = rootDir+"\\item\\" + new Date().getTime()+"_.json";
            if(!System.IO.File.Exists(path)){
                oSession.utilDecodeResponse();
                oSession.SaveResponseBody(path);
        ...
        if(oSession.url.Contains("/fxc/services/ms/fxc/hb_Makerlogo.jsp")){//生成二维码海报。注意是jpeg图片流
            //从链接中获取产品id，该id将作为文件名
            ///fxc/services/ms/fxc/hb_Makerlogo.jsp?url=https%3A%2F%2Ffx3-xinan.lhs11.com%2Ffxc%2Fh10%2Fp241099.html%3FproductState%3D2%26p%3D31384493%26s%3D31384493
            var regex = /p(\d+)\.html/;
            //var productIds = regex.test(oSession.url);
            var productIds = oSession.url.match(regex);
            var path = rootDir+"\\qrcode\\" + productIds[1] +".jpeg";
            if(!System.IO.File.Exists(path)){
                oSession.utilDecodeResponse();
                oSession.SaveResponseBody(path);
                //TODO:可以调用pan执行transform转换完成数据提交
