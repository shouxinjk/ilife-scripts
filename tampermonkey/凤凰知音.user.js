...
// @match        http://ffp.airchina.com.cn/app/product/detail*
// @match        https://ffp.airchina.com.cn/app/product/detail*
// @grant        none
// ==/UserScript==

//示例URL
//http://ffp.airchina.com.cn/app/product/detail?id=30866
...
(function() {
    'use strict';
    waitForKeyElements (".name h1", title);
    waitForKeyElements (".nch-breadcrumb span a[href*=searchType]", category);
    waitForKeyElements (".goods-sub-title", summary);
    ...
    waitForKeyElements ("#imageMenu img", images_slide);
    waitForKeyElements (".tpjz img", images);
    waitForKeyElements ("strong[nctype='goods_price']",price_sale);
...
function props(jNode){
    var key = jNode.find(".w-detail-name").text().trim();
    var value = jNode.find(".w-detail-con").text().trim();
...
//http://ffp.airchina.com.cn/app/product/detail?id=30866
function web(link){
    var id = link.match(/\d+/g);//匹配后返回：30866
    return "https://ffp.airchina.com.cn/app/product/detail?id="+id[0];
}

//转换为移动链接地址
//web: http://ffp.airchina.com.cn/app/product/detail?id=30866
//wap: http://ffp.airchina.com.cn/app/product/detail?id=30866
function wap(link){
    var id = link.match(/\d+/g);//匹配后返回：30866
    return "https://ffp.airchina.com.cn/app/product/detail?id="+id[0];
}
