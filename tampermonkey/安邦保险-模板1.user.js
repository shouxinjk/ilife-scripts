...
// @match        https://www.ab95569.com/product/Jump.do?prono=*
// @grant        none
// ==/UserScript==

//示例URL
//https://www.ab95569.com/product/Jump.do?prono=0687
...
(function() {
    'use strict';
    waitForKeyElements (".tit h2", title);
    waitForKeyElements (".tit p", tags_summary);
    //waitForKeyElements (".tit p", category);
    waitForKeyElements (".pic img", images_slide);
    waitForKeyElements (".abf_pdbox img", images);
    //waitForKeyElements ("a[href*=storeId]", seller);
    waitForKeyElements ("dt:contains('：')",props);
...
function props(jNode){
    var txt = jNode.text().replace(/：/g,"").trim();
    var valueEl = jNode.parent().find("dd:first");
    if(valueEl && txt.length>0){
        var key = txt;
...
//https://www.ab95569.com/product/Jump.do?prono=0687
function web(link){
    var id = link.match(/prono=\d+/g);//匹配后返回：prono=0687
    return "https://www.ab95569.com/product/Jump.do?PRODUCTID".replace(/PRODUCTID/,id);
}
