...
// @match        http://jf.ccb.com/exchangecenter/product/materialproductdetail.jhtml?productId=*
// @grant        none
// ==/UserScript==

//示例URL
//http://jf.ccb.com/exchangecenter/product/materialproductdetail.jhtml?productId=1380018
//this.$ = this.jQuery = jQuery.noConflict(true);//important: to remove jQuery conflict

var debug = false;//isProduction("ccb");
...
(function() {
    'use strict';
    waitForKeyElements (".xq_title", title);
    waitForKeyElements (".st1 a", category);
    waitForKeyElements (".picName img", images_slide);
    waitForKeyElements (".xq_pic_img p span img", images);
    waitForKeyElements (".xq_pic_img p", summary);
    //waitForKeyElements ("a[href*=storeId]", seller);
    waitForKeyElements (".p-parameter-list li",props);
    waitForKeyElements (".product-points",price_sale);
...
//http://jf.ccb.com/exchangecenter/product/materialproductdetail.jhtml?productId=1380018
function web(link){
    var id = link.match(/productId=\d+/g);//匹配后返回：productId=1380018
    return "http://jf.ccb.com/exchangecenter/product/materialproductdetail.jhtml?PRODUCTID".replace(/PRODUCTID/,id);
}

//转换为移动链接地址
//web: http://jf.ccb.com/exchangecenter/product/materialproductdetail.jhtml?productId=1380018
//wap: http://jf.ccb.com/exchangecenter/product/materialproductdetail.jhtml?productId=1380018
function wap(link){
    var id = link.match(/productId=\d+/g);//匹配后返回：productId=1380018
    return "http://jf.ccb.com/exchangecenter/product/materialproductdetail.jhtml?PRODUCTID".replace(/PRODUCTID/,id);
}
