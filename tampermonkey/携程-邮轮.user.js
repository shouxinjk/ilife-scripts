...
// @match        https://cruise.ctrip.com/*
// @grant        none
// ==/UserScript==

//示例URL
//https://cruise.ctrip.com/p/804276.html?departurecityid=104#departure=2019-06-23&sellerid=35430_24335
if(this.jQuery){
...
function extract(){
    ...
    params();//从链接中提取标签
    waitForKeyElements (".detail_title>span", title);
    //waitForKeyElements (".scores_litem dd", tags);
    waitForKeyElements (".s_label", tags_product);
    waitForKeyElements (".crumbs a", tags_crumb);
    waitForKeyElements (".nav_list li img", images_slide);
    waitForKeyElements (".sum_review_num span",rank_score);
    waitForKeyElements (".reviews_filter_box span[data-section='All']",rank_count);
    waitForKeyElements (".detail_price .base_price",price_sale);//售价
    waitForKeyElements (".detail_price del",price_bid);
    waitForKeyElements (".supplier",supplier);
    waitForKeyElements (".seller",seller);
    //waitForKeyElements (".pro-intro span",props);
    waitForKeyElements ("#markdown p", summary);
    waitForKeyElements (".slide_img img", images);
}
...
function tags_crumb(jNode){
    var tag = jNode.text().replace('预订','').trim();
    if(tag.trim().length>2 && tag.trim().length<10 && data.tags.indexOf(tag)<0){
        data.tags.push(tag);
    }
    commit("tags_crumb");
...
function images_slide(jNode){
    var img = fullUrl(jNode.attr("src").replace(/120_80/,"580_375"));
    checkImage(img);
}
...
function supplier(jNode){
    data.producer.name = jNode.text().trim().replace(/\s+/g,"").replace(/本产品由【/,"").replace(/】提供岸上游服务/,"");//供应商：携程自营
    commit("supplier");
}

function seller(jNode){
    data.seller.name = jNode.text().trim().replace(/\s+/g,"");//供应商：携程自营
    commit("seller");
}

//转换为web链接地址：需要去除附加信息，确保唯一性
//原始: https://cruise.ctrip.com/p/804276.html?departurecityid=104#departure=2019-06-23&sellerid=35430_24335
//转换后: https://cruise.ctrip.com/p/804276.html
function web(link){
    var id = link.match(/\/\w+\/\d+/);//p/804276,/c/10359
    return "https://cruise.ctrip.com"+id+".html";
}

//转换为移动链接地址
//原始: https://cruise.ctrip.com/p/804276.html?departurecityid=104#departure=2019-06-23&sellerid=35430_24335
//wap: https://m.ctrip.com/webapp/cruise/p/804276.html?departurecityid=104&jump=0&#departure=2019-06-23&sellerid=35430_24335
function wap(link){
    var id = link.match(/\/\w+\/\d+/);//p/804276,/c/10359
    return "https://m.ctrip.com/webapp/cruise"+id+".html";
}
