...
// @match        http://www.quyou.com/play/play.aspx?id=*
// @grant        none
// ==/UserScript==

//示例URL
//http://www.quyou.com/play/play.aspx?id=3249
if(this.jQuery){
...
function extract(){
    if(debug){console.log("extract activity ...");}
    params();//从链接中提取标签
    waitForKeyElements ("#playTitle", title);
    waitForKeyElements ("#plFeature", summary);
    waitForKeyElements ("#lblType", tags);
    waitForKeyElements ("#playOrigin", tags);
    waitForKeyElements ("#playDestination", summary_destination);
    waitForKeyElements ("#playTraffic", tags);
    waitForKeyElements ("#playAbode", tags);
    //waitForKeyElements (".prop-text", tags_product);
    waitForKeyElements ("#playImg img", images_slide);
    waitForKeyElements (".body img", images);
    ...
    waitForKeyElements ("#playPrice",price_sale);//售价
...
function summary_destination(jNode){
    data.summary += "目的地："+jNode.text().replace(/\s+/g," ")+"<br/>";
    commit("summary");
}
...
function price_sale(jNode){
    var price = jNode.text().replace(/,/g,"").match(/\d+\.*\d*/g)[0];
...
function web(link){
    ...
    return "http://www.quyou.com/play/play.aspx?"+id;
}

//转换为移动链接地址
//web: http://www.quyou.com/play/play.aspx?id=3249
function wap(link){
    var id = link.match(/id=\d+/);//id=580931148092
    return "http://www.quyou.com/play/play.aspx?"+id;
}
