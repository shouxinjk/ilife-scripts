...
// @match        http://www.tuniu.com/package/*
// @match        http://www.tuniu.com/drive/*
// @match        http://www.tuniu.com/tour/*
// @grant        none
// ==/UserScript==

//示例URL
//http://www.tuniu.com/package/210759948
//http://www.tuniu.com/drive/210731092
//http://www.tuniu.com/tour/210586988
if(this.jQuery){
...
var data={
    ...
    type:type(window.location.href),
    category:[],
    source:"tuniu",
    title:null,
    summary:"",
...
function extract(){
    ...
    params();//从链接中提取标签
    waitForKeyElements (".resource-title", title);
    waitForKeyElements (".resource-recommend-content-inner", summary);
    waitForKeyElements (".resource-tag", tags);
    waitForKeyElements (".crumb a", tags_crumb);
    waitForKeyElements (".gallery-photo img", images_slide);
    waitForKeyElements (".detail-feature-photos img", images);
    waitForKeyElements (".comment-satisfaction",rank_score);
    waitForKeyElements (".detail-tab-item[data-rel='#J_Comment']",rank_count);
    waitForKeyElements (".price-content del",price_bid);//原价
    waitForKeyElements (".price-number",price_sale);//售价
...
function title(jNode){
    var txt = jNode.text().replace("<","").split(">");
    data.title = txt[0];
    if(txt.length>1){
