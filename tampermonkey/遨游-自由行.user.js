...
// @match        http://www.aoyou.com/domesticpackage/p*
// @grant        none
// ==/UserScript==

//示例URL
//http://www.aoyou.com/domesticpackage/p121431i1
if(this.jQuery){
...
function extract(){
    ...
    waitForKeyElements (".icon-zyx", tags);
    waitForKeyElements (".tag-default", tags_product);
    waitForKeyElements (".crumbs-nav li a", tags_crumb);
    waitForKeyElements (".productInfoBox-picGetReady li img", images_slide);
    waitForKeyElements ("#span_goodPercent",rank_score);
    waitForKeyElements ("li:contains('累计销量：')",rank_count);
    waitForKeyElements ("#showPrce",price_sale);//售价
    waitForKeyElements (".tag-zy",supplier);//供应商
    waitForKeyElements ("#innerText p", summary);
    waitForKeyElements (".cpdetail_xcjs img", images);
}
...
function images_slide(jNode){
    var img = fullUrl(jNode.attr("src").replace(/w\/90\/h\/70/,"w/568/h/420"));
    checkImage(img);
}
...
function rank_score(jNode){
    ...
    data.rank.score = score&&score[0]?Number(Number(score[0])/20).toFixed(1):null;
    data.rank.base = 5;
    commit("rank_score");
}
...
//原始: http://www.aoyou.com/domesticpackage/p121431i1?***
//转换后: http://www.aoyou.com/domesticpackage/p121431i1
function web(link){
    var id = link.match(/domesticpackage\/[a-z0-9]+/);
    return "http://www.aoyou.com/"+id;
}

//转换为移动链接地址
//web: http://www.aoyou.com/domesticpackage/p121431i1
//wap: https://m.aoyou.com/tourstatic.html?pid=121431&i1&noapp=1
function wap(link){
    var ids = link.match(/\d+/g);//匹配后返回：121431，1
    return "https://m.aoyou.com/tourstatic.html?pid=_product&i_category"
