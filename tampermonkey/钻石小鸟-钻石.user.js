...
// @match        http://www.zbird.com/diamond/*
// @grant        none
// ==/UserScript==

//示例URL
//http://www.zbird.com/diamond/6370784.html
...
(function() {
    ...
    waitForKeyElements (".show_pic_diam img", images_slide);
    ...
    waitForKeyElements (".u_psys_pri:contains('价格') b:first",price_sale);
    waitForKeyElements (".u_psys_pri:contains('特惠') b:first",price_sale);//两者出现一种
    waitForKeyElements (".u_psys_linepri:contains('￥') b",price_bid);
    waitForKeyElements (".u_diam_sys_tb td",props);
...
function props(jNode){
    if(jNode.find("em") ){
        var key = jNode.find("em").text().trim();
        var value = jNode.text().replace(key,"").trim();
        var prop = {
            //key:key,
            //value:value
        };
...
function images_slide(jNode){
    var img = fullUrl(jNode.attr("src")).replace("/150/","/400/");
    checkImage(img);
}
