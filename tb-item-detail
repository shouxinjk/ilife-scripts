...
// @require http://118.190.75.121/list/js/waitForKeyElements.js
...
function patchDocument(documentId){
    ...
    //step 2: update data
    req.open('PATCH', 'https://data.pcitech.cn/_db/sea/beautiful/things/'+documentId, true);
...
function createDocument(){
    ...
    req.open('POST', 'https://data.pcitech.cn/_db/sea/beautiful/things', true);
    req.setRequestHeader('Content-Type', 'application/json');
    req.setRequestHeader('Api-Key', 'foobar');
    req.onreadystatechange = function() {
        if (req.readyState === 4) {
...
function getNextItem(){
    ...
    document.location.href="https://item.taobao.com/item.htm?spm=a219r.lm5734.14.324.581d86fbFmejKL&id=565630134195&ns=1&abbucket=1#detail";
}
